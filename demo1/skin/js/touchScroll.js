...
function MScroll(objName,sDirection,bBar,bMove)
{
	...
	this.iMaxT=sDirection=="x"?this.oScroll.offsetWidth-this.obj.clientWidth:this.oScroll.offsetHeight-this.obj.clientHeight;
	...
	if(bBar)
	{
		...
		this.oBar.style.height=this.scrollWrap/this.scale+"px";
	}
...
MScroll.prototype={
	fnStart:function(e)
	{
		...
		if(this.iScroll>50 || this.iScroll<-this.iMaxT-50)
		{
			this.bOver=true;
		}
		else
	...
	fnMove:function(e)
	{
		...
		{
			this.iScroll=50;
			if(this.bOver&&this.onOverUp)
			{
				this.oScroll.addEventListener("touchend",overUp,false);
			}
		}
		if(this.iScroll<-this.iMaxT-50)
		{
			this.iScroll=-this.iMaxT-50;
			if(this.bOver&&this.onOverDown)
			{
				this.oScroll.addEventListener("touchend",overDown,false);
			}
		}
		this.setCss();
		function overDown()
		{
			_this.onOverDown();
			_this.oScroll.removeEventListener("touchend",overDown,false);
		}
		function overUp()
		{
			_this.onOverUp();
			_this.oScroll.removeEventListener("touchend",overUp,false);
		}
	},
	fnEnd:function()
	{
		
		var iDis=(this.lastDis-this.dis)*4;
	...
	tweenMove:function(iTarget,sType,fnBack,bMove)
	{
		if(typeof bMove == "undefined")
		{
			bMove=this.bMove;
